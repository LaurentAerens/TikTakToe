...
public abstract class SearchEngineBase : IEngine
{
    ...
    public (int[,] Board, int Score) Move(int[,] board, int player, int? depth = null)
    {
        ...
        {
            throw new BoardSizeNotSupportedException(this.GetType().Name, board.GetLength(0), board.GetLength(1));
        }
        ...
        return this.StartSearch(board, player, useDepth);
    }
    ...
    public int Eval(int[,] board, int player, int? depth = null)
    {
        ...
        var (_, score) = this.SearchRecursive(board, player, useDepth, player);
        return score;
    }
    ...
    private (int[,] Board, int Score) StartSearch(int[,] board, int player, int depth)
    {
        ...
        if (moves.Count > 1)
        {
            ...
            {
                var (_, moveScore) = this.SearchRecursive(moves[i], ChangePlayer(player), depth - 1, enginePlayer);
                scores[i] = moveScore;
            });
        ...
        var (_, singleMoveScore) = this.SearchRecursive(moves[0], ChangePlayer(player), depth - 1, enginePlayer);
